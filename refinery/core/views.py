...
def admin_test_data(request):
    ...
    project_objects = []
    # create projects (for each user: private, lab shared read/write,
    # project group shared read-only, public shared)
    for user_object in user_objects:
        # # PRIVATE PROJECT
        project_name = user_object.first_name + "\'s Private Project"
        project_summary = "A project that is only visible to " + \
        ...
        project_object = Project.objects.create(
            name=project_name, summary=project_summary)
        ...
        project_summary = "A project that is owned by " + \
                          user_object.first_name + \
                          " and shared for reading with the general public."
        # delete if exists
        project_object = Project.objects.filter(
            name__exact=project_name, summary=project_summary)
        if project_object is not None:
            project_object.delete()
        ...
        project_summary = "A project that is owned by " + \
                          user_object.first_name + \
                          " and shared for reading with the " \
                          "\'Refinery Project\' ExtendedGroup."
        # delete if exists
        project_object = Project.objects.filter(name__exact=project_name)
        if project_object is not None:
            project_object.delete()
        ...
        project_summary = "A project that is owned by " + \
                          user_object.first_name + \
                          " and shared for reading and writing their lab " \
                          "ExtendedGroup."
        # delete if exists
        project_object = Project.objects.filter(name__exact=project_name)
        if project_object is not None:
    ...
    # create data_sets (for each user: private, lab shared read/write,
    # data_set group shared read-only, public shared)
    for user_object in user_objects:

        # # PRIVATE data_set
        data_set_name = user_object.first_name + "\'s Private Data Set"
        data_set_summary = "A data set that is only visible to " + \
                           user_object.first_name + "."
        # delete if exists
        ...
        data_set_object = DataSet.objects.create(
            name=data_set_name, summary=data_set_summary)
        ...
        data_set_summary = "A data set that is owned by " + \
                           user_object.first_name + \
                           " and shared for reading with the general public."

        # delete if exists
        data_set_object = DataSet.objects.filter(
            name__exact=data_set_name, summary=data_set_summary)
        if data_set_object is not None:
            data_set_object.delete()
        ...
        data_set_summary = "A data_set that is owned by " + \
                           user_object.first_name + \
                           " and shared for reading with the " \
                           "\'Refinery Project\' group."
        # delete if exists
        data_set_object = DataSet.objects.filter(name__exact=data_set_name)
        if data_set_object is not None:
            data_set_object.delete()
