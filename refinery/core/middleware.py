...
class ExternalToolErrorMiddleware(object):
    def process_request(self, request):
        if not re.search('/admin/', request.path):
            # check celery
            celery_tuple = check_tool_status(
            ...
            if celery_tuple[0]:
                if celery_tuple[1] == ExternalToolStatus.TIMEOUT_STATUS:
                    context_dict = {
                        ...
                        'message_start':
                            "Our database has not been updated with the "
                            "status of Celery recently leading us to believe "
                            "that something"
                    }
                    response = render_to_response(
                        'external_tool_down.html',
                    ...
                    context_dict = {
                        ...
                        'message_start':
                            "Something in Celery, we know not what,"
                    }
                    response = render_to_response(
                        'external_tool_down.html',
                        context_dict,
            ...
            if solr_tuple[0]:
                if solr_tuple[1] == ExternalToolStatus.TIMEOUT_STATUS:
                    context_dict = {
                        ...
                        'message_start':
                            "Our database has not been updated with the "
                            "status of Solr recently leading us to believe "
                            "that something"
                    }
                    response = render_to_response(
                        'external_tool_down.html',
            ...
            for workflow_engine in WorkflowEngine.objects.all():
                ...
                galaxy_tuple = check_tool_status(
                    ExternalToolStatus.GALAXY_TOOL_NAME,
                    tool_unique_instance_identifier=instance.api_key
                )
                if galaxy_tuple[0]:
                    if galaxy_tuple[1] == ExternalToolStatus.TIMEOUT_STATUS:
                        context_dict = {
                            ...
                            'message_start':
                                "Our database has not been updated with the "
                                "status of Galaxy recently leading us to "
                        ...
                        context_dict = {
                            'external_tool_name': "Unknown",
                            'message_start':
                                "Something in Galaxy, we know not what,"
                        }
                        response = render_to_response(
                            'external_tool_down.html',
                            context_dict,
