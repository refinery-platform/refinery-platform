...
class progress_chord(object):
    ...
    def __call__(self, body, **options):
        ...
        r = self.Chord.apply_async(
            (list(self.tasks), body), self.options, **options)
...
def run_analysis_preprocessing(analysis):
    ...
    try:
        ...
        error_msg += "error importing workflow into Galaxy for "
        error_msg += "analysis '%s': %s"
        logger.error(error_msg, analysis.name, exc.message)
        analysis.set_status(Analysis.FAILURE_STATUS, error_msg)
        analysis.cleanup()
        run_analysis_preprocessing.update_state(state=celery.states.FAILURE)
...
def monitor_analysis_execution(analysis):
    ...
    # get a fresh copy of analysis to avoid potential issues with parallel
    # execution
    try:
        analysis = Analysis.objects.get(uuid=analysis.uuid)
    ...
    try:
        ...
        monitor_analysis_execution.update_state(state=celery.states.FAILURE)
        logger.error(
            "AnalysisStatus object does not exist for analysis '%s': " +
            "unable to monitor",
            analysis_status)
        return
    ...
    if progress["message"]["error"] > 0:
        ...
        # check if we have enough data sets in history
