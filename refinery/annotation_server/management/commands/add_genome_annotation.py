...
class Command(BaseCommand):   
    ...
    def mappabilityTheoretical(self, bed_file):
        ...
        '''
        logger.debug("annotation_server.mappabilityTheoretical called for genome: %s, file: %s" % (self.GENOME_BUILD, bed_file))
        if self.GENOME_BUILD == 'hg19':
            db_table = self.GENOME_BUILD + "_MappabilityTheoretical"
            self.addBedAnnotation(bed_file, db_table)
        else:
            raise CommandError('Theoretical Mappability Annotation track only supported for hg19 currently')
    ...
    def mappabilityEmpirical(self, bed_file):
        ...
        params: annotation_file 
        '''
        logger.debug("annotation_server.mappabilityEmpirical called for genome: %s, file: %s" % (self.GENOME_BUILD, bed_file))
        db_table = self.GENOME_BUILD + "_MappabilityEmpirial"
        if (self.GENOME_BUILD == 'hg19') or (self.GENOME_BUILD == 'dm3') or (self.GENOME_BUILD == 'ce10') :
            self.addBedAnnotation(bed_file, db_table)
        else:
            raise CommandError('Empirical Mappability Annotation track only supported for hg19 currently')
    ...
    def gapRegions(self, bed_file):  
        ...
        params: annotation_file 
        '''
        logger.debug("annotation_server.gapRegions called for genome: %s, file: %s" % (self.GENOME_BUILD, bed_file))
        ...
        if (self.GENOME_BUILD == 'hg19') or (self.GENOME_BUILD == 'dm3'):
            ...
            raise CommandError('GapRegions Annotation track only supported for hg19 and dm3')
        
    
    def geneAnnotation(self, gff_file):
        '''
        Adding additional annotation tracks from Gencode / Wormbase / Flybase based on the GFF file format
        params: annotation_file 
        '''
        logger.debug("annotation_server.addGeneAnnotation called for genome: %s, file: %s" % (self.GENOME_BUILD, gff_file))
        ...
        if self.GENOME_BUILD == 'hg19':
            ...
            db_vars = ['gene_id','transcript_id','gene_type','gene_status','gene_name','transcript_type','transcript_status','transcript_name']
    ...
    def addGeneAnnotation(self, gff_file, db_model, table_vals):
        '''
        Function for adding additional gene annotation files i.e hg19 gencode, dm3 flybase, ce10 wormbase
        '''
        current_table = eval(db_model)
    ...
    def addBedAnnotation(self, bed_file, db_model):
        ...
        '''
        logger.debug("annotation_server.addBedAnnotation called for genome: %s, file: %s table: %s" % (self.GENOME_BUILD, bed_file, db_model))
        ...
        for line in handle:
            if line[0] != '#':
                t1 = line.strip().split('\t')
                item = current_table(chrom=t1[0], chromStart=t1[1], chromEnd=t1[2])
                item.save()
