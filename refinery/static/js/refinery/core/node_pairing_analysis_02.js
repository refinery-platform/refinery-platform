	...
	*/
	...
	nrMod.workflowCollectionView = Marionette.CollectionView.extend({
		...
		changeWorkflow: function(event) {
			...
			App.vent.trigger("change_inputs", this.workflow_inputs); // => alert box "bar"
		},
	...
	//===================================================================
	 ...
	 nrMod.npController = Marionette.Controller.extend({
		...
		initialize: function(options){
			...
			var self = this;
			this.model_nodes = new nrMod.nodeSetCollection();
        ...
        start: function(options){
			...
			//var defer = [];
            //defer.push(this.model_nodes.fetch())
			//defer.push(this.model_workflow.fetch())
			//this.defer = defer;
			App.vent.trigger("begin_loaded");

			//$.when.apply($, this.defer).done(nrMod.startApp);
        ...
        show: function(){
			...
			App.p_workflow.show(this.nr_wcv);
	...
	nrMod.addInitializer(function(){
		...
		//$.when.apply($, deferreds).done(function() { console.log("succesesdfadfasdf called"); App.vent.trigger("begin_loaded");} );
		console.log("-------");
		App.vent.trigger("begin_loaded");

		//$.when.apply($, deferreds).done(function() { console.log("succesesdfadfasdf called");} );

		var controller = new nrMod.npController();
	    //controller.start();
	    console.log(nrApp);
