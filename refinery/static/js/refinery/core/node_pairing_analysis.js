...
//analyzeApp.module('npApp', function(npApp, App, Backbone, Marionette, $, _){
	...
	var nodeSetsModel = Backbone.Collection.extend({
	...
	var workflowsModel= Backbone.Collection.extend({
	    urlRoot: '/api/v1/workflow/',
	    model: workflowModel,
	...
	var inputRelationshipView = Marionette.ItemView.extend({
		...
		render: function() { 
			...
			if (this.current_inputs != '') {
				...
				var temp_html = '<form class="form-horizontal"><fieldset><div class="control-group">';
					...
					if (w2.set1 != null) {
						...
						<div class="controls"> <select id="dropdown_set1" name="set1">'
						 + temp_options + '</select>';
						 	...
						 	temp_html += '<a id="run_analysis_single_btn" href="#" onclick="runSingleAnalysis()" role="button" class="btn btn-warning" data-toggle="modal" rel="tooltip" data-placement="" data-html="true" data-original-title="Launch IGV with&lt;br&gt;current selection."><i class="icon-dashboard"></i>&nbsp;&nbsp;Run Analysis</a></div></div>';
...
var deferreds = [];
var model_ns = new nodeSetsModel();
var model_in = new workflowsModel();
