...
PivotMatrix = function( elementId, options, matrix, facets, xPivot, yPivot, useGradient, updateCallback ) {
	...
	if ( this.useGradient ) {
		...
		// binary color map
		defaultColor = d3.scale.linear().domain([0,1]).range(["lightgray", "black"]).clamp(true);		
		selectedColor = d3.scale.linear().domain([0,1]).range(["yellow", "red"]).clamp(true);	
	}
	...
	/*
	// lines are generated by making the cells smaller
	row.append("line")
		.attr("x1", 0 )
		.attr("x2", width )
		.attr("y1", 0 )
