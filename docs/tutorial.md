...
__This tutorial can be completed using the Tutorial Data Set, which consists of sample ChIP-seq data and their associated metadata file (need links). Follow all steps (except creating a metadata file as it already exists) and note the bold bullet points below some steps with expanded instructions specific to the Tutorial Data Set.__
