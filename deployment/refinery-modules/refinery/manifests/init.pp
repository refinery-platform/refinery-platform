class refinery {
  ...
  class solr {
    ...
    exec { "solr_install_as_service":
      command => "sudo bash /usr/src/install_solr_service.sh /usr/src/${solr_archive} -d ${project_root}/solr_app_data -u ${app_user} && chown -R ${app_user}:${app_user} /opt/solr-${solr_version}",
      creates => "/opt/solr-${solr_version}",
      path => "/usr/bin:/bin",
    }
    ->
