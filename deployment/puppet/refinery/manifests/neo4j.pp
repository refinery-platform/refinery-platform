class refinery::neo4j (
  ...
  $ontology_plugin_url = "https://github.com/refinery-platform/neo4j-ontology/releases/download/v0.5.0/ontology.jar"
  ...
  # already exists.
  exec { "neo4j_ontology_plugin_download":
    command => "rm -f /var/lib/neo4j/plugins/ontology.jar && wget -P /var/lib/neo4j/plugins/ ${ontology_plugin_url}",
  ...
  $owl2neo4j_url = "https://github.com/flekschas/owl2neo4j/releases/download/v0.6.1/owl2neo4j.jar"
  # Need to remove the old file manually as wget throws a weird
  # `HTTP request sent, awaiting response... 403 Forbidden` error when the file
  # already exists.
  exec { "owl2neo4j_wget":
